...
echo $OUTPUT->header();
echo '<h2>'.get_string('status_report', 'block_learning_plan').': '.format_string(get_learningplan_name($lp_id), false).'</h2>';
echo '<p>'. get_string('report_at', 'block_learning_plan').' '.
    userdate(time(),get_string('strftimedatetime', 'core_langconfig')).'</p>';
...
echo html_writer::table($student_status);
        //echo '</div>';
    //}
$PAGE->requires->js_init_call('M.block_learning_plan.init', array("", ""));
echo $OUTPUT->footer();
//End Form Display